...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for l in range(self.num_layers - 1):
            self.params['W' + str(l)] = np.random.randn(last_layer_dims, hidden_dims[l]) * weight_scale
            self.params['b' + str(l)] = np.zeros(hidden_dims[l])

            last_layer_dims = hidden_dims[l]

        last_num_str = str(self.num_layers - 1)
        self.params['W' + last_num_str] = np.random.randn(last_layer_dims, num_classes) * weight_scale
        self.params['b' + last_num_str] = np.zeros(num_classes)
    ...
    def loss(self, X, y=None):
        ...
        for l in range(self.num_layers - 1):
            if self.normalization=='batchnorm':
                ...
                out, cache = affine_relu_forward(last_layer_out, self.params['W' + str(l)], self.params['b' + str(l)])
        ...
        last_num_str = str(self.num_layers - 1)
        scores, cache = affine_forward(last_layer_out, self.params['W' + last_num_str], self.params['b' + last_num_str])
        ...
        loss_reg_sum = 0
        for l in range(self.num_layers - 1):
            loss_reg_sum += np.sum(self.params['W' + str(l)] ** 2)

        loss += 0.5 * self.reg * loss_reg_sum

        # gradients

        dx, dw, db = affine_backward(dx, caches[-1])
        grads['W' + last_num_str] = dw + self.reg * self.params['W' + last_num_str]
        grads['b' + last_num_str] = db
